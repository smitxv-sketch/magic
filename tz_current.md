...
*   **Движок:** Рендерит контент на основе `src/data/kaizen_show.json`.
*   **Режиссёрский сценарий (Director's Cut):**
    *   Скрытый слой для спикера, вызываемый по клику на иконку "Info" (появляется при наведении на заголовок).
    *   Содержит: `speakerScript` (текст выступления) и `hiddenQnA` (ответы на сложные вопросы).
*   **Типы блоков:**
    *   `hero`: Главный экран.
