...
export const TenantLogo: React.FC<TenantLogoProps> = ({
  ...
  const handleError = () => {
    ...
    if (nextIndex < pathsToTry.length) {
      ...
      const text = fallbackText || config?.name || "Logo";
      setCurrentSrc(
        `https://placehold.co/200x80/e2e8f0/64748b?text=${encodeURIComponent(text)}`,
      );
    }
  };
