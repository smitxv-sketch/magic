...
export const InteractiveDemoBlock = ({ block }: { block: PresentationBlock }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [showDirectorsCut, setShowDirectorsCut] = useState(false);

  // Mapping block IDs to Scenario IDs
  const scenarioIdMap: Record<string, string> = {
    'demo-secretary': 'incoming_letter',
    'demo-lawyer': 'contract_review',
    'demo-shadow-audit': 'shadow_audit'
  };

  const scenarioId = scenarioIdMap[block.id];
  const scenario = scenarioId ? getScenarioById(scenarioId) : undefined;

  // Global listener for Director's Cut is handled in PresentationView, 
  // but we can also add a hidden trigger here if needed.
